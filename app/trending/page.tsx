...
import { getMoviesByCategory, getSeriesByCategory, type Movie, type Series } from "@/lib/firebase-db"
    ...
    async function fetchTrending() {
      try {
        const [movies, series] = await Promise.all([
          getMoviesByCategory("trending", 25),
          getSeriesByCategory("trending", 25),
        ])
        ...
        <main className="flex-1 px-4 lg:px-6 py-6">
          <div className="mb-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl lg:text-4xl font-bold">Trending Now</h1>
            </div>
            <p className="text-muted">What everyone is watching right now</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-muted">
              <p>No trending content available yet. Check back soon!</p>
            </div>
          ) : (
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-6 2xl:grid-cols-7 gap-3 lg:gap-4 pb-24 lg:pb-8">
              {content.map((item) => (
